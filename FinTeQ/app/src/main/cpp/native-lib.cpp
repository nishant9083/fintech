...
Java_com_example_task2_MainActivity2_libint(JNIEnv *env, jobject mainActivityInstance, jint fileDescriptor) {
    cleanUp();
...
Java_com_example_task2_MainActivity2_login(JNIEnv *env, jobject mainActivityInstance, jstring jStr) {
...
JNIEXPORT jstring JNICALL
Java_com_example_task2_MainActivity2_readCertificate(JNIEnv *env, jobject mainActivityInstance) {
    dlhandle = dlopen("liblsusbdemo.so", RTLD_LAZY);
    if (dlhandle == NULL) {
        return env->NewStringUTF("Failed to load library");
    }
...
JNIEXPORT jstring JNICALL
Java_com_example_task2_MainActivity2_logout(JNIEnv *env, jobject thiz) {
    dlhandle = dlopen("liblsusbdemo.so", RTLD_LAZY);
    if (dlhandle == NULL) {
        return env->NewStringUTF("Failed to load library");
    }
